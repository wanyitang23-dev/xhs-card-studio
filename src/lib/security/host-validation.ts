 ...
 *   `XHS_ANYTHING_ALLOWED_HOSTS` — comma-separated list (e.g. for a LAN-host
 *      setup or a `.local` mDNS name). Each entry matches case-insensitively
 *      against the bare hostname; port is ignored.
 *   `XHS_ANYTHING_ALLOW_ANY_HOST=1` — opt-out wildcard, intended for behind
...
/**
 * Parse `XHS_ANYTHING_ALLOWED_HOSTS` into a normalized, lowercased set. Empty
...
export function isRequestHostAllowed(req: { headers: { get(name: string): string | null } }): boolean {
  return isAllowedHost(req.headers.get("host"), {
    extraAllowed: parseAllowedHosts(process.env.XHS_ANYTHING_ALLOWED_HOSTS),
    allowAny: process.env.XHS_ANYTHING_ALLOW_ANY_HOST === "1",
  });
}
