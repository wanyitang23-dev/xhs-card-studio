...
describe("isRequestHostAllowed (env-driven wrapper)", () => {
  ...
  it("extends allowlist via XHS_ANYTHING_ALLOWED_HOSTS", () => {
    process.env.XHS_ANYTHING_ALLOWED_HOSTS = "html.anything.lan";
    expect(isRequestHostAllowed(make("html.anything.lan:3000"))).toBe(true);
    expect(isRequestHostAllowed(make("attacker.example"))).toBe(false);
  });
  it("opt-out wildcard via XHS_ANYTHING_ALLOW_ANY_HOST=1 accepts everything", () => {
  ...
  it("envVar=0 stays strict (only '1' opts out)", () => {
    process.env.XHS_ANYTHING_ALLOW_ANY_HOST = "0";
    expect(isRequestHostAllowed(make("attacker.example"))).toBe(false);
  });
});
