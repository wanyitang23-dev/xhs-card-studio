...
/**
 * User-installed and user-uploaded templates live outside the repo so they
 * survive `git clean` and aren't accidentally committed. Default location:
 * `~/.xhs-anything/skills/`.
 *
 * Deliberately *not* `~/.html-anything/` — this is a standalone tool, and
 * sharing that directory would push xhs-only templates into the upstream app's
 * picker (and vice versa).
 *
 * Tests override via `XHS_ANYTHING_USER_SKILLS_DIR`.
 */
export function userSkillsDir(): string {
  const override =
    process.env.XHS_ANYTHING_USER_SKILLS_DIR ?? process.env.XHS_ANYTHING_USER_SKILLS_DIR;
  if (override) return override;
  return path.join(os.homedir(), ".xhs-anything", "skills");
}
