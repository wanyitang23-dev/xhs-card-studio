...
describe("isHostAllowed", () => {
  const original = {
    allowed: process.env.XHS_ANYTHING_ALLOWED_HOSTS,
    any: process.env.XHS_ANYTHING_ALLOW_ANY_HOST,
  };

  beforeEach(() => {
    delete process.env.XHS_ANYTHING_ALLOWED_HOSTS;
    delete process.env.XHS_ANYTHING_ALLOW_ANY_HOST;
  });

  afterEach(() => {
    if (original.allowed === undefined) delete process.env.XHS_ANYTHING_ALLOWED_HOSTS;
    else process.env.XHS_ANYTHING_ALLOWED_HOSTS = original.allowed;
    if (original.any === undefined) delete process.env.XHS_ANYTHING_ALLOW_ANY_HOST;
    else process.env.XHS_ANYTHING_ALLOW_ANY_HOST = original.any;
  });
  ...
  it("allows a hostname listed in XHS_ANYTHING_ALLOWED_HOSTS", () => {
    process.env.XHS_ANYTHING_ALLOWED_HOSTS = "ha.local, my-box";
  ...
  it("accepts any host when XHS_ANYTHING_ALLOW_ANY_HOST=1 (trusted-proxy mode)", () => {
    process.env.XHS_ANYTHING_ALLOW_ANY_HOST = "1";
...
describe("hostRejectedResponse", () => {
  it("returns a 403 JSON response with an actionable hint", async () => {
    ...
    expect(body.hint).toContain("XHS_ANYTHING_ALLOWED_HOSTS");
  });
});
