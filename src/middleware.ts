 ...
 * env knobs (`XHS_ANYTHING_ALLOWED_HOSTS`, `XHS_ANYTHING_ALLOW_ANY_HOST`).
 *
...
export function middleware(req: NextRequest) {
  ...
  return new NextResponse(
    JSON.stringify({
      ...
      hint:
        "this tool's API only accepts requests with a loopback Host header (127.0.0.1, localhost, ::1). " +
        "If you're fronting it behind a different hostname, add it to XHS_ANYTHING_ALLOWED_HOSTS (comma-separated) " +
        "or set XHS_ANYTHING_ALLOW_ANY_HOST=1 if a trusted reverse proxy is terminating Host upstream.",
    }),
    {
      status: 403,
...
// Pin to the Node runtime so `process.env.XHS_ANYTHING_ALLOWED_HOSTS` and
// `process.env.XHS_ANYTHING_ALLOW_ANY_HOST` are read per-request, not
...
// legitimate LAN hosts (`XHS_ANYTHING_ALLOWED_HOSTS`) or failing to disable
// the gate (`XHS_ANYTHING_ALLOW_ANY_HOST=1`). Node runtime middleware
